...
function linkify(text) {
    ...
    return text.replace(urlRegex, function(url) {
        return '<a style="color:#4498CB;" href="' + url + '">' + url + '</a>';
    });
}
...
function emojiParser(name) {
  var words = name.split(" ");
  for(var i=0;i< words.length;i++) {
    if(words[i].trim().startsWith(":") && words[i].trim().endsWith(":")) {
      var ghImg = emoji_data[words[i].trim().split(":")[1]]
      if(ghImg) {
        var imgUrl = `<img class="arcadia-emoji" src="${ghImg}">`
        name = name.replace(words[i], imgUrl)
      } else {
        continue
      }
    }
...
/**
console.log(
 emojiParser("Hello :smile: :smiley:")
)
**/
